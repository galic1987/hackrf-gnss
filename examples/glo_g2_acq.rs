//! GLONASS L2OF (G2) FDMA acquisition: the same 511-chip code as L1OF, on the
//! L2 channel plan 1246.0 MHz + k*0.4375 MHz. Mirrors glonass_acq.rs.
//! usage: glo_g2_acq <iq> <fs> <fc> [secs]
use hackrf_gnss::glonass::{glonass_code, l2_freq, CHIP_RATE};
...
fn main() {
    ...
    println!("GLONASS L2OF FDMA search, {:.1}s:", secs);
    let mut hits = 0;
    for k in -7..=6 {
        let ifhz = l2_freq(k) - fc;
        ...
        sig = decimate(&sig, q);
        let fsd = fs / q as f64;
        let r = acquire_codes(&sig, fsd, &[(0usize, code.clone())], CHIP_RATE, &dop, nb, 2.5, l2_freq(k), true);
        let m = r[0].metric;
        if m > 2.5 {
            hits += 1;
        }
    ...
    println!("=> {} GLONASS L2 channels with a satellite", hits);
}
