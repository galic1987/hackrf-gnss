...
impl HackRf {
    ...
    pub fn open_by_index(index: usize) -> Result<Self> {
        Self::open_filtered(index, None)
    }

    /// Open a HackRF device by serial number string (as printed by
    /// hackrf_info / list_devices).
    pub fn open_by_serial(serial: &str) -> Result<Self> {
        Self::open_filtered(0, Some(serial))
    }

    fn open_filtered(index: usize, serial: Option<&str>) -> Result<Self> {
        ...
        if let Some(want) = serial {
            devices.retain(|d| d.serial_number() == Some(want));
        }
        let devices = devices;
