...
- Understand RF switch circuit topologies: Skyworks SKY13322 / MASWSS0129 and series DC-blocking capacitors.
- Grasp the physics of passive antennas vs unpowered active antennas (LNA attenuation).
...
1. **Switch Matrix Architecture & DC Constraints:**
   - 2 primary ports (PA, PB), 8 secondary ports (A1–A4, B1–B4), $1\text{ MHz}\text{--}4000\text{ MHz}$.
   - Series DC-blocking capacitors ($C_1, C_2, C_5, C_6, C_9\text{--}C_{12}$) isolate RF pins, preventing internal SDR bias-T $3.3\text{V}$ power from reaching active antennas without external bias-T injectors.
2. **Empirical Antenna Classification by Spectral Profiling:**
   - Wideband non-emitting sweep across $50\text{ MHz}\text{--}2000\text{ MHz}$ to classify antenna passbands, notches (e.g. Mohu FM filter), and LNA attenuation states.
3. **Polarimetric Multipath Separation:**
...
4. **Automated ABBA Receiver Delay Calibration:**
...
5. **In-Situ Y-Factor Radiometer Calibration:**
...
### Practical Repo Exploration
- Study [`docs/operacake_integration_plan.md`](file:///Volumes/Radiator%208TB/gnss/hackrf_gnss/docs/operacake_integration_plan.md) and [`scripts/operacake_sweep_survey.py`](file:///Volumes/Radiator%208TB/gnss/hackrf_gnss/scripts/operacake_sweep_survey.py).
- Inspect empirical spectral profiles in `/Volumes/Radiator 8TB/gnss/observations/operacake_sweep_summary.json`.
- Control Opera Cake from the terminal: `hackrf_operacake -d <serial> -a A1`.
...
### Lab 3: Multi-Port RF Antenna Survey and Active vs Passive Front-End Diagnostics
Using the HackRF Opera Cake and `scripts/operacake_sweep_survey.py`:
1. Connect at least 3 distinct antennas (e.g., a passive whip, a TV antenna with an amplifier/notch filter, and an unpowered active GPS antenna).
2. Execute an automated 50–2000 MHz sweep without RF emissions (`-a 0 -p 0`).
3. Compute band-averaged power across FM, VHF Airband, UHF TV, and GNSS L1.
4. Prove that unpowered active antennas exhibit $>15\text{ dB}$ of out-of-band attenuation relative to passive antennas due to cold LNA transistors.
